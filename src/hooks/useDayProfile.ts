 ...
 */
export function useDayProfile(now: Date, lat: number, lon: number, timeZone: string): DayProfile {
  const dateKey = dateKeyInZone(now, timeZone);

  return useMemo(() => sampleDay(dateKey, lat, lon, timeZone), [dateKey, lat, lon, timeZone]);
}
