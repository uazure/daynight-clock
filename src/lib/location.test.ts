...
function stubDeviceZone(zone: string, offsetName?: string): void {
  ...
  function impl(locales?: Intl.LocalesArgument, options?: Intl.DateTimeFormatOptions): Intl.DateTimeFormat {
    if (!options?.timeZone) {
      return {
  ...
  vi.spyOn(Intl, 'DateTimeFormat').mockImplementation(impl as unknown as typeof Intl.DateTimeFormat);
}
