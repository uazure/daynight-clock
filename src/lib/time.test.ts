...
describe('wallClockInZone', () => {
  it('reads the wall clock of an instant in a given zone', () => {
    ...
    expect(wallClockInZone(instant, 'Asia/Kathmandu')).toEqual({
      year: 2026, month: 1, day: 15, hour: 17, minute: 45, second: 0,
    })
  ...
  it('crosses the date line without touching the device zone', () => {
    ...
    expect(wallClockInZone(instant, 'Asia/Tokyo')).toEqual({
      year: 2026, month: 1, day: 16, hour: 8, minute: 30, second: 0,
    })
    expect(wallClockInZone(instant, 'America/New_York')).toEqual({
...
describe('instantForZoneWallClock', () => {
  it('inverts wallClockInZone for ordinary times', () => {
    expect(instantForZoneWallClock('2026-07-04', 12, 30, 'America/New_York')).toEqual(
      new Date('2026-07-04T16:30:00Z'),
    )
    expect(instantForZoneWallClock('2026-01-15', 17, 45, 'Asia/Kathmandu')).toEqual(
      new Date('2026-01-15T12:00:00Z'),
    )
    expect(instantForZoneWallClock('2026-01-15', 0, 0, 'UTC')).toEqual(
  ...
  it('picks one of the two instants of a fall-back overlap', () => {
    ...
    expect([
      new Date('2026-10-25T00:30:00Z').getTime(),
      new Date('2026-10-25T01:30:00Z').getTime(),
    ]).toContain(instant.getTime())
...
describe('hoursSinceMidnightInZone', () => {
  it('converts a zone wall time to fractional hours', () => {
    expect(
      hoursSinceMidnightInZone(new Date('2026-07-04T16:30:00Z'), 'America/New_York'),
    ).toBeCloseTo(12.5, 9)
    expect(
      hoursSinceMidnightInZone(new Date('2026-01-15T12:00:00Z'), 'Asia/Kathmandu'),
    ).toBeCloseTo(17.75, 9)
    expect(hoursSinceMidnightInZone(new Date('2026-01-15T00:00:00Z'), 'UTC')).toBeCloseTo(
      0,
      9,
    )
  })
  ...
  it('approaches 24 just before the zone midnight', () => {
    expect(
      hoursSinceMidnightInZone(new Date('2026-01-15T23:59:59.999Z'), 'UTC'),
    ).toBeCloseTo(24, 4)
  })
})
