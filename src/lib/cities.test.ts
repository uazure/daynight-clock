...
import { type City, cityToPlace, offsetsDiffer, searchCities } from './cities';
...
describe('searchCities', () => {
  ...
  it('ranks a whole-name prefix above a later-word match', () => {
    expect(searchCities(CITIES, 'york').map((c) => c.name)).toEqual(['York', 'New York City']);
  });
  ...
  it('ignores diacritics in the query', () => {
    expect(searchCities([city('Malmo', 'SE')], 'malmö').map((c) => c.name)).toEqual(['Malmo']);
  });
