...
const rows = readFileSync(source, 'utf8')
  ...
  .filter((row) => row.name && row.tz && Number.isFinite(row.lat) && Number.isFinite(row.lon))
...
const write = (file, data) => writeFileSync(join(outDir, file), `${JSON.stringify(data)}\n`, 'utf8');
